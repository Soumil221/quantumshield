...
# ── Phase 5 — Header scanning models ────────────────────────────────────────
class HeaderPortResult(BaseModel):
    port: int
...
class HeaderAnalysis(BaseModel):
    host: str
    port_results: list[HeaderPortResult] = Field(default_factory=list)
    overall_grade: str = Field("unknown")


class HeaderScanResponse(BaseModel):
    status: str = Field("success")
    total_hosts_scanned: int = Field(...)
    total_ports_scanned: int = Field(...)
    results: list[HeaderAnalysis] = Field(default_factory=list)
